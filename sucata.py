...
def Apontamento_Sucata():
    ...
    # Converter as colunas 'Sucata' e 'Peso' para float (removendo separador de milhar)
    df_corte_filtrado['Sucata'] = pd.to_numeric(
        df_corte_filtrado['Sucata'].str.replace('.', '', regex=False).str.replace(',', '.'),
        errors='coerce'
    )
    df_corte_filtrado['Peso'] = pd.to_numeric(
        df_corte_filtrado['Peso'].str.replace('.', '', regex=False).str.replace(',', '.'),
        errors='coerce'
    )
    ...
    if df_corte_filtrado['Aprov.'].dtype == 'object':
        df_corte_filtrado['Aprov.'] = pd.to_numeric(
            df_corte_filtrado['Aprov.'].str.replace('.', '', regex=False).str.replace(',', '.'),
            errors='coerce'
        )
    ...
    df_filtrado_por_data['Sucata'] = pd.to_numeric(
        df_filtrado_por_data['Sucata'].str.replace('.', '', regex=False).str.replace(',', '.'),
        errors='coerce'
    )
    df_filtrado_por_data['Peso'] = pd.to_numeric(
        df_filtrado_por_data['Peso'].str.replace('.', '', regex=False).str.replace(',', '.'),
        errors='coerce'
    )
    ...
    if df_filtrado_por_data['Aprov.'].dtype == 'object':
        df_filtrado_por_data['Aprov.'] = pd.to_numeric(
            df_filtrado_por_data['Aprov.'].str.replace('.', '', regex=False).str.replace(',', '.'),
            errors='coerce'
        )
